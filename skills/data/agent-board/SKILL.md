---
name: agent-board
description: Use it to build a persistent data application — dashboard, metrics view, internal reporting tool — on the AgentBoard runtime, by writing SQL migrations and a declarative JSON specification instead of application code. Reach for it whenever the user wants a dashboard, admin panel, internal tool, or "somewhere to see this data" backed by SQLite, and whenever an `agent-board.json` or a `workspaces/` directory is present. Do not use it for querying a database the user already runs, or for a UI that needs custom components.
---
...
- **Migrations are the only way to write data.** There is no insert or seed command.
  Rows you want present at startup go in a migration alongside the DDL.
